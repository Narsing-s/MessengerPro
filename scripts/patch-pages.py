...
if 'const getApiBase = () =>' not in s:
    if marker not in s:
        raise SystemExit('Account type marker missing')
...
login_replacement = r'''async function submitLogin() {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json(); const account = { ...data.account, password };
        localStorage.setItem('messengerpro.account', JSON.stringify(account)); localStorage.setItem('messengerpro.session', 'true');
        if (remember) localStorage.setItem('messengerpro.remember', 'true'); onLogin(account); return;
      }
      if (response.status !== 404) { const data = await response.json().catch(() => ({})); return setNotice(data.error || 'Invalid credentials.'); }
    } catch {}
    const accounts = getAccounts(); const key = value.trim().toLowerCase();
    const account = accounts.find((a) => (mode === 'email' ? a.email.toLowerCase() === key : mode === 'username' ? a.username.toLowerCase() === key : a.phone === value.replace(/\D/g, '')) && a.password === password);
    if (!account) return setNotice('Invalid credentials. Check your details or use Forgot password.');
    localStorage.setItem('messengerpro.account', JSON.stringify(account)); localStorage.setItem('messengerpro.session', 'true'); onLogin(account);
  }'''
s, n = re.subn(login_pattern, login_replacement, s, count=1, flags=re.S)
if n != 1:
    raise SystemExit('submitLogin function not found')
...
register_replacement = r'''async function createAccount() {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json(); const created = { ...data.account, password };
        const existing = getStoredAccounts().filter((a) => a.username.toLowerCase() !== created.username.toLowerCase());
        localStorage.setItem('messengerpro.accounts', JSON.stringify([...existing, created])); localStorage.setItem('messengerpro.account', JSON.stringify(created)); localStorage.setItem('messengerpro.session', 'true');
        setNotice('Account created successfully. Opening your Messenger workspace…'); setTimeout(() => onLogin(created), 250); return;
      }
      const data = await response.json().catch(() => ({}));
      if (response.status !== 404) return setNotice(data.error || 'Unable to create account.');
    } catch {}
    const accounts = getStoredAccounts();
    if (accounts.some((a) => a.username.toLowerCase() === account.username.toLowerCase())) return setNotice('That username is already in use.');
    if (accounts.some((a) => a.email.toLowerCase() === account.email.toLowerCase())) return setNotice('That email is already in use.');
    if (accounts.some((a) => a.phone === account.phone)) return setNotice('That mobile number is already in use.');
    localStorage.setItem('messengerpro.accounts', JSON.stringify([...accounts, account])); localStorage.setItem('messengerpro.account', JSON.stringify(account)); localStorage.setItem('messengerpro.session', 'true'); setNotice('Account created successfully. Opening your Messenger workspace…'); setTimeout(() => onLogin(account), 500);
  }'''
s, n = re.subn(register_pattern, register_replacement, s, count=1, flags=re.S)
if n != 1:
    raise SystemExit('createAccount function not found')
...
if 'const [directory, setDirectory]' not in s:
    if state_marker not in s:
        raise SystemExit('Home state marker missing')
...
if "apiFetch(`/api/users/search" not in s:
    if hook_marker not in s:
        raise SystemExit('toast hook marker missing')
...
if n != 1:
    s = s.replace("const availableContacts = sampleContacts;", new_dir, 1)
